...
def evaluate(model_path, X_test, y_test, beam_width=10):
    ...
    mdFile.new_list(items=['Prediction: ' + str(np.array(seq[0]) + 1) + '\tLog Likelihood: ' + str(seq[1]) for seq in result]) 
    replace = False
    result = beam_search_decoder(pred_latest, beam_width, replace)
    mdFile.new_line("Beam Width: {} replace {}".format(beam_width, replace)  )
    mdFile.new_list(items=['Prediction: ' + str(np.array(seq[0]) + 1) + '\tLog Likelihood: ' + str(seq[1]) for seq in result]) 
    ...
    mdFile.new_table(columns=2, rows=len(pred)+1, text=list_of_strings, text_align='center')
