...
functions exist. We build three of them below, each fitting all 1,629 draws to machine
precision. The failure, when it comes, is not philosophical hand-waving: it is a specific,
measurable step, and this page measures it.
...
| **Polynomial** | degree-5 polynomial through the last six draws | 7×10⁻¹³ |

No strawmen: these genuinely reproduce every past draw. Any of them, asked about a past
date, answers perfectly. The premise "a function that fits perfectly on the past draws" is
delivered in full.
...
| Sequence | lzma size | vs the 23.96-bit floor |
|---|---:|---:|
| The real archive (1,629 draws) | 24.30 bits/draw | **100.4%** — incompressible |
| 200 simulated fair histories | 24.30 bits/draw mean | identical to the real one |
| A control actually generated by a simple function | **2.50 bits/draw** | **10.4%** |
